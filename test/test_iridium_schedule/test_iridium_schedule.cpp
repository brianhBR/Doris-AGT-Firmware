...
void test_failed_unlocated_attempt_retries_after_the_interval(void) {
    IridiumSchedule_reset(&sched);
    IridiumSchedule_noteAttempt(&sched, 1000UL, false);
...
void test_failed_located_attempt_retries_after_the_interval(void) {
    IridiumSchedule_reset(&sched);
    IridiumSchedule_noteAttempt(&sched, 1000UL, true);
...
void test_failed_located_upgrade_waits_before_retrying(void) {
    IridiumSchedule_reset(&sched);
    IridiumSchedule_noteSent(&sched, 120000UL, false);
    TEST_ASSERT_TRUE(IridiumSchedule_locatedDue(&sched, 120001UL,
                                                REPORT_INTERVAL_MS));

    IridiumSchedule_noteAttempt(&sched, 120001UL, true);
    TEST_ASSERT_FALSE(IridiumSchedule_locatedDue(
        &sched, 120002UL, REPORT_INTERVAL_MS));
    TEST_ASSERT_TRUE(IridiumSchedule_locatedDue(
        &sched, 120001UL + REPORT_INTERVAL_MS, REPORT_INTERVAL_MS));
}
